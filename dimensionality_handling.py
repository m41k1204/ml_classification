...
def main():
    ...
    print("="*50)
    X_train_ig, X_test_ig, ig_indices = information_gain_selection(X_train_norm_A, y_train_encoded_A, X_test_norm_A, k=25)
    X_train_ig_bias = add_bias(X_train_ig)
    X_test_ig_bias = add_bias(X_test_ig)
    
    w_ig, loss_train_ig, loss_test_ig = softmax_regression.train(X_train_ig_bias, y_train_encoded_A, 
                                                                 epochs, alpha, 
                                                                 x_test=X_test_ig_bias, y_test=y_test_encoded_A)
    y_pred_ig, acc_ig = softmax_regression.test(X_test_ig_bias, y_test_encoded_A, w_ig)
    analyze_results(y_test_encoded_A, y_pred_ig, "Information Gain - Softmax Regression")
    
    # 2. Random Forest Feature Importance
    print("="*50)
    print("RANDOM FOREST FEATURE IMPORTANCE")
    print("="*50)
    X_train_rf, X_test_rf, rf_indices = random_forest_feature_importance(X_train_norm_A, y_train_encoded_A, X_test_norm_A, k=25)
    X_train_rf_bias = add_bias(X_train_rf)
    X_test_rf_bias = add_bias(X_test_rf)
    
    w_rf, loss_train_rf, loss_test_rf = softmax_regression.train(X_train_rf_bias, y_train_encoded_A, 
                                                                 epochs, alpha, 
                                                                 x_test=X_test_rf_bias, y_test=y_test_encoded_A)
    y_pred_rf, acc_rf = softmax_regression.test(X_test_rf_bias, y_test_encoded_A, w_rf)
    analyze_results(y_test_encoded_A, y_pred_rf, "Random Forest - Softmax Regression")
    
    # 3. PCA
    print("="*50)
    print("PCA")
    print("="*50)
    X_train_pca, X_test_pca, variance_explained = pca_dimensionality_reduction(X_train_norm_A, X_test_norm_A, n_components=30)
    X_train_pca_bias = add_bias(X_train_pca)
    X_test_pca_bias = add_bias(X_test_pca)
    print(f"Varianza explicada acumulada con 22 componentes: {variance_explained[-1]:.4f}")
    
    w_pca, loss_train_pca, loss_test_pca = softmax_regression.train(X_train_pca_bias, y_train_encoded_A, 
                                                                    epochs, alpha, 
                                                                    x_test=X_test_pca_bias, y_test=y_test_encoded_A)
    y_pred_pca, acc_pca = softmax_regression.test(X_test_pca_bias, y_test_encoded_A, w_pca)
    analyze_results(y_test_encoded_A, y_pred_pca, "PCA - Softmax Regression")
    
    # Resumen de resultados
    print("="*50)
    print("RESUMEN DE RESULTADOS")
    print("="*50)
    print(f"Information Gain: {acc_ig:.2f}%")
    print(f"Random Forest: {acc_rf:.2f}%")
    print(f"PCA: {acc_pca:.2f}%")
